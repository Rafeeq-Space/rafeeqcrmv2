...
const CAMPAIGN_STATUS_ORDER: CampaignStatus[] = ['draft', 'active', 'paused', 'ended']
...
function EditCampaignModal({
  ...
  const [teamIds, setTeamIds] = useState<string[]>(campaign.team_ids || [])
  const toggleTeam = (id: string) => setTeamIds(prev => prev.includes(id) ? prev.filter(t => t !== id) : [...prev, id])
  const [form, setForm] = useState({
    name: campaign.name,
  ...
  const [sources, setSources] = useState<CampaignSource[]>(
    campaign.sources?.length ? campaign.sources : [campaign.source]
  )
  const [tags, setTags] = useState<string[]>(campaign.tags || [])
  const [tagInput, setTagInput] = useState('')
  const [links, setLinks] = useState<KnowledgeLink[]>(campaign.links || [])
  const [linkForm, setLinkForm] = useState({ label: '', url: '' })
  const [files, setFiles] = useState<KnowledgeFile[]>(campaign.files || [])
  const [images, setImages] = useState<string[]>(campaign.images || [])
  const [uploading, setUploading] = useState(false)
  const [saving, setSaving] = useState(false)
  ...
  async function uploadFile(file: File, folder: 'files' | 'images'): Promise<string> {
    const supabase = createClient()
    const ext = file.name.split('.').pop()
    const path = `${campaign.tenant_id}/campaigns/${folder}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`
    await supabase.storage.from('knowledge').upload(path, file, { upsert: true })
    const { data } = supabase.storage.from('knowledge').getPublicUrl(path)
    return data.publicUrl
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.name) return
    if (sources.length === 0) { setError('اختر منصة واحدة على الأقل'); return }
    setSaving(true)
    setError('')
    const supabase = createClient()
      ...
      <div className="modal p-6 w-full max-w-2xl my-8" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted2 mt-1.5">تغيير الفِرَق هنا لا يعيد توزيع العملاء الحاليين — فقط يتحكم بمن يظهر كخيار توزيع بالنماذج الجديدة.</p>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="btn btn-outline flex-1">إلغاء</button>
            <button type="submit" disabled={saving || uploading} className="btn btn-primary flex-1">{saving ? 'جارٍ الحفظ...' : 'حفظ التعديلات'}</button>
          </div>
        </form>
      </div>
    </div>
...
function CampaignDetailModal({
  campaign, forms, isAdmin, getFormLink, copied, onCopyLink, onCreateForm, onDeleteForm, onViewSheet, onEdit, onClose,
}: {
  campaign: Campaign
  forms: Form[]
  isAdmin: boolean
      ...
      <div className="modal p-6 w-full max-w-2xl my-8" onClick={e => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-1 shrink-0">
            {isAdmin && (
              <button onClick={onEdit} className="text-muted2 hover:text-foreground p-1" aria-label="تعديل الحملة"><Pencil size={18} /></button>
            )}
            <button onClick={onClose} className="text-muted2 hover:text-foreground p-1"><X size={20} /></button>
          </div>
        </div>
