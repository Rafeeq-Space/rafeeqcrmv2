...
// Three independent client_admin-only actions on the leads page: export a
// one-off .xlsx, save a permanent archive snapshot (see the /archive page),
// or delete every lead outright. See:
...
export default function LeadsAdminActions({ leadCount }: Props) {
  ...
  async function deleteAll() {
    ...
    setDeleteError('')
    try {
      const res = await fetch('/api/client-admin/leads/delete-all', { method: 'POST' })
      const body = await res.json().catch(() => ({}))
          ...
          <div className="card w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold flex items-center gap-2" style={{ color: 'var(--danger)' }}>
                <AlertTriangle size={18} /> تأكيد حذف كل العملاء
              </h3>
              <button onClick={closeModal} className="text-muted2 hover:text-danger" disabled={deleting}><X size={18} /></button>
            </div>

            <p className="text-sm text-muted">
              سيتم حذف <strong className="text-foreground">{leadCount}</strong> عميل نهائيًا مع كل بياناتهم (المكالمات، الملاحظات، التعيينات). هذا الإجراء لا يمكن التراجع عنه ولا يقوم بأي تصدير تلقائي — استخدم &quot;تصدير Excel&quot; أو &quot;الأرشيف&quot; الأول لو محتاج نسخة.
            </p>
